...
Validation harness in place. See `CONTRIBUTING.md` for the current
five-step process used to add each family, including the **step 0
bd_warehouse gate** — anything bd_warehouse already covers is
explicitly out of scope.

> **Future direction**: the steady-state workflow for this library
> is described in
...
> — parametric classes generated by `fcstd2b123d` from declarative
> YAML manifests, not hand-written Python. The current hand-written
> workflow is the bootstrap; phases 1-4 of the design doc replace it
> with deterministic translator-driven generation.
